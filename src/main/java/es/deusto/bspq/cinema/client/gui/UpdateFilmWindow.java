...
public class UpdateFilmWindow extends JFrame {

	private JFrame frmUpdateFilm;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UpdateFilmWindow window = new UpdateFilmWindow();
					window.centreWindow();
					window.frmUpdateFilm.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	 ...
	 */
	public UpdateFilmWindow() {
		setResizable(false);
		initialize();
		centreWindow();
		frmUpdateFilm.setVisible(true);
	}
	...
	private void initialize() {
		frmUpdateFilm = new JFrame();
		frmUpdateFilm.setType(Type.UTILITY);
		frmUpdateFilm.setTitle("Update Film");
		frmUpdateFilm.setBounds(100, 100, 450, 300);
		frmUpdateFilm.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		
		panelFormulary = new JPanel();
		panelFormulary.setBorder(new TitledBorder(null, "Update Film", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		frmUpdateFilm.getContentPane().add(panelFormulary, BorderLayout.CENTER);
		...
		frmUpdateFilm.getContentPane().add(panelButton, BorderLayout.SOUTH);
		
		JButton btnUpdateFilm = new JButton("Update Session");
		btnUpdateFilm.setEnabled(false);
		panelButton.add(btnUpdateFilm);
	}
